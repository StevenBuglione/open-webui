    ...
    openwebui = {
      ...
      secrets = concat([
        ...
        },
        {
          ...
          value_from = aws_secretsmanager_secret.openwebui_oidc_client.arn
        },
        {
          name       = "SCIM_TOKEN"
    ...
    litellm = {
      ...
      command = [
        trimspace(<<-EOT
          ...
          CONFIG_CONTENT="$${LITELLM_CONFIG:-$${LITELLM_CONFIG_FALLBACK:-}}"
          if [ -z "$CONFIG_CONTENT" ]; then
            echo "LiteLLM config not provided via secret or fallback" >&2
            exit 1
          fi
          printf '%s\n' "$CONFIG_CONTENT" > "$CONFIG_PATH"
          echo "LiteLLM resolved configuration:" >&2
          cat "$CONFIG_PATH" >&2
          exec litellm --config "$CONFIG_PATH" --port ${var.litellm_container_port} --host 0.0.0.0
        EOT
        )
      ]
      secrets = concat([
        {
          name       = "LITELLM_CONFIG"
          value_from = aws_secretsmanager_secret.litellm_config.arn
        }
      ], var.litellm_secrets)
    }
    mcpo = {
      ...
      desired_count   = var.enable_mcpo ? 1 : 0
      cpu             = 512
      memory          = 1024
      attach_to_alb   = var.enable_mcpo
      path_patterns   = ["/mcpo/*"]
      env             = var.mcpo_env
      entry_point     = ["/bin/sh", "-c"]
      command = [
        trimspace(<<-EOT
          set -euo
          CONFIG_PATH=/tmp/mcpo-config.json
          pip install --no-cache-dir "mcpo>=0.0.11"
          printf '%s' "$MCPO_CONFIG" > "$CONFIG_PATH"
          exec mcpo --config "$CONFIG_PATH" --port ${var.mcpo_container_port} --host 0.0.0.0
        EOT
        )
      ]
      secrets = concat([
        {
          name       = "MCPO_CONFIG"
          value_from = aws_secretsmanager_secret.mcpo_config.arn
        }
      ], var.mcpo_secrets)
    }
  }
...
module "compute" {
  ...
  depends_on                = [aws_acm_certificate_validation.managed]
}

resource "aws_iam_role_policy" "task_bedrock_access" {
  name = "${local.name}-bedrock-access"
  role = module.compute.task_role_name
